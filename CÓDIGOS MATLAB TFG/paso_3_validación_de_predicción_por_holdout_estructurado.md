Paso 3: validación de la predicción por holdout estructurado.

 Sobre los tensores del paso 1, se entrena el modelo con una sub-rejilla completa de 4×4×4 (64 casos, tomando los niveles alternos de cada resistencia) y se predicen los 279 casos restantes de la base de datos, no vistos durante el entrenamiento. La predicción del HOSVD, con tres modos retenidos por dirección e interpolación de las matrices de factores, se compara con dos métodos de referencia: una SVD matricial, que desdobla el tensor en la forma Q1 × (Q2, Q3), y la interpolación tridimensional de tipo spline. Para cada salida y método se calculan el RMSE, el error máximo absoluto y el R², que se imprimen por consola. El script genera cuatro figuras: la comparativa de error por método (RMSE relativo y error máximo en escala logarítmica), la predicción frente al CFD para la temperatura máxima (a la izquierda solo el HOSVD y a la derecha los tres métodos), la distribución de los errores de T_max y una superficie tridimensional T_max(Q1, Q2) con Q3 fijo que enfrenta CFD y HOSVD. De estas salidas gráficas proceden las Figuras 51 y 52 de la memoria. Los resultados numéricos se guardan en resultados_prediccion.mat.
